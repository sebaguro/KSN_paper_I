...
\address[wsu]{
   Western Sydney University, Kingswood campus, NSW, Australia (visiting fellow 2019, 2024)
}
...
revealing path-dependence incompatible with the independent-increment
multiplicative structure with positive drift required by Kardashev's
...
10^{15}$\,yr: $\sim 10^5$ times both the age of the Universe
and the main-sequence lifetime of the Sun.
...
increase in human energy expenditure was projected at 3--4\% 
over the following 60 years \citep{Putnam1948} on the basis 
of statistical findings. However,
...
limitation that has received little quantitative attention.
\citet{kar64}, \citet{sagan73}, and \citet{Shklovskii1966} noted
that an advanced civilisation must be characterised not only by 
its energy throughput but by the \emph{information richness} of 
...
global energy production \citep{Ritchie2022} --- used here as a
...
the Antikythera mechanism two millennia ago.
 ...
 Roughly two thousand years after the construction of this earliest known analogue computer, Charles Babbage and Alan Turing (AT)
are widely recognised as the modern 'fathers' of the digital computer
(Swade, 2017). The pioneering work of AT paved the foundations of
Artificial Intelligence (AI) and Cryptography --- two streams whose
natural confluence is now visible in agentic AI systems that reason
over cryptographically secured information substrates.
...
we present the independence property argument as a further
...
see \citet{Smil2017} for a comprehensive historical analysis of the 
industrial-era energy trajectory.
...
$P(1965) = 4.95\,\mathrm{TW}$, in close agreement with the \citet{kar64}
...
Kardashev conjecture \citep{kar64}, so that
...
\citet{kar64} proposed that the energy production of a technologically
...
\citet{kar64} calculated that a Type~II civilisation --- defined as
...
approximately year 5188\,CE. \citet{sagan73} obtained a similar value
...
baseline of the data (Figure~\ref{fig:ksn_panel1}). Figure~\ref{fig:ksn_zoom} 
shows the same data and fits on a linear vertical axis, making the 
remarkably low dispersion directly visible: the linear and exponential 
models are visually near-identical across 1965--2024, while the 
Kardashev one-percent model diverges strikingly downward from the data.
...
probable than an independent-increment multiplicative generator with
positive drift would predict, and their occurrence is traceable to specific
historical events rather than symmetric statistical fluctuations.

A further structural objection to $(1+x)^t$ with $x > 0$ is 
that the model is sign-constrained: every year must exhibit 
positive growth by construction. The observed series contains 
identifiable years of negative growth (e.g., 2008 and 2020) that 
the model cannot accommodate at any parameter value --- a 
structural, not merely statistical, falsification of the 
always-positive growth assumption that is independent of the 
...
\citep{Metropolis1953,Hastings1970} with a prior 
$r \sim \mathcal{N}(0.01,\, 0.02^2)$ centred on the Kardashev value
and $N = 60{,}000$ posterior samples after a $15{,}000$-step burn-in. 
The burn-in phase is discarded because the first iterations of the 
chain are influenced by the arbitrary starting point in parameter 
space rather than by the target posterior; the Markov property of 
the sampler ensures that the chain eventually forgets this initial 
condition and converges to the stationary distribution, from which 
the retained samples are drawn. The
...
\begin{enumerate}
    ...
    \item The year-over-year differences $\Delta P$ are non-Gaussian
          ...
          independent-increment multiplicative generator with
          positive drift.
...
accomplishes. As \citet{sagan73} noted, a civilisation that wastes energy
...
and information is the Landauer limit \citep{Landauer1961,Bennett1982}: the 
minimum energy required to erase one bit of information at temperature $T$ 
is $k_B T \ln 2 \approx 2.85 \times 10^{-21}\,\mathrm{J}$ at room temperature. The ratio
...
global measure of computational proof-of-work \citep{nak2009},
fulfilling the requirement of \citet{sagan73} that the Kardashev state
...
\textbf{Concurrent work.} Complementary work from the quantum-computing 
perspective has recently examined Bitcoin mining as a Kardashev-scale 
problem \citep{DallaireDemers2026}, demonstrating that quantum-accelerated 
Bitcoin mining at current network difficulty would require a fleet of 
$\sim 10^{23}$ physical qubits drawing $\sim 10^{25}\,\mathrm{W}$ --- 
resources consistent with a Kardashev Type~II civilisation. Their result 
places an upper bound on quantum-mining feasibility: the resource 
requirements are so extreme that practical quantum mining collapses under 
hardware, error-correction, and fleet-logistics overheads. Our analysis 
is methodologically distinct --- we use the Bitcoin hashrate as an 
empirical proxy for global classical proof-of-work and renormalise the 
Kardashev state variable accordingly --- but shares the use of 
proof-of-work infrastructure as a natural bridge between civilisational 
energy and irreversible computation.

\textit{Adjacent technosphere literature.}~A parallel research programme has approached the limits of civilisational growth from the perspective of technosignatures and technosphere modelling. \citet{Wright2022Case} synthesise the longstanding SETI-community argument that technosignatures may be more abundant, longer-lived, and more detectable than biosignatures, motivating renewed interest in quantitative frameworks for civilisational scale. 
\citet{HaqqMisra2025Scenarios} developed ten self-consistent scenarios for Earth's technosphere over a thousand-year horizon — three with zero-growth stability, two that have collapsed, one that oscillates, and four that continue to grow — illustrating that conventional Kardashev-style continuous-growth extrapolation captures only a subset of plausible long-term futures.

\citet{HaqqMisra2025Luminosity} argue that the Kardashev scale is better understood as a "luminosity limit" — the maximum capacity for a civilisation to harvest luminous stellar energy within a given spatial domain — and that thermodynamic efficiency keeps any luminosity-limited technosphere strictly below this theoretical maximum; they further consider hypothetical "stellivore" technospheres that might exceed the luminosity limit by harvesting stellar mass directly.

\citet{Balbi2025WasteHeat} analyse, via simple thermodynamic models, the constraints that unavoidable waste-heat generation imposes on the long-term habitability of Earth-like planets hosting technospheres in persistent exponential growth, demonstrating that the loss of habitable surface conditions can be expected on timescales of $\lesssim 10^{3}$~yr from the onset of the exponential phase, for an assumed annual energy-consumption growth rate of $\sim 1\%$ --- the same conservative Kardashev rate that the present work statistically falsifies.

\citet{Cirkovic2018GreatSilence} situates such constraints within a four-category taxonomy of Fermi-paradox solutions, whose "logistic" category groups hypotheses in which Galactic sophonts exist and are in principle detectable but the cost of maintaining detectability over astrophysical timescales is prohibitive; the waste-heat and luminosity-limit arguments above provide specific physical mechanisms by which this cost might become prohibitive.

The present analysis is complementary to these works: rather than projecting forward from a model of future growth, we statistically falsify backward from six decades of observed data, and we propose a variable transformation~--- rather than a new functional form~--- as the resolution of the resulting Conundrum. The KSN renormalisation $\mathscr{B} = P/\mathcal{H}$ approaches the Landauer limit precisely in the regime where waste-heat dissipation per bit of irreversible computation is minimised, connecting it naturally to both constraints.

Moreover, the present analysis adds a further entry to \citeauthor{Cirkovic2018GreatSilence}'s logistic category: because no $P(t)$ functional form can simultaneously satisfy statistical adequacy and physical coherence, the Type II threshold of the original Kardashev scale~--- defined as $P \to L_\odot \sim 3.8 \times 10^{26}$~W via energy scaling~--- is unreachable on any physically coherent timescale. A Type II energy-scaling technosignature, in the Dyson-sphere or full-stellar-luminosity-harvesting sense, is correspondingly unlikely ever to be observed~--- though the KSN reframing as the approach $\mathscr{B} \to k_{B} T \ln 2$ retains the classificatory structure of the Kardashev scale on a different state variable.
...
intent of \citet{kar64} and the information-theoretic extension
proposed by \citet{sagan73}.
...
arises from probability theory. Kardashev's geometric series $(1+x)^t$ 
implicitly models year-over-year growth factors as independent draws 
from a distribution with mean $(1+x)$. For a non-negative random variable 
$X$ with this property, the memoryless relation holds:
\begin{equation}
    P(X > a + b \mid X > b) \;=\; P(X > a)\,.
    \label{eq:memoryless}
\end{equation}
That is: the fractional increment $\Delta P / P$ is independent of
the current level and of history. The global energy system, however, 
manifestly possesses memory: infrastructure lock-in, geopolitical 
constraints, technology diffusion rates, and crisis events (e.g., 2008, 2020) 
all induce path-dependence in the year-over-year increments, violating 
the independence assumption that $(1+x)^t$ requires.
...
not symmetric statistical fluctuations drawn from an independence-preserving 
distribution. An independent-increment multiplicative generator with positive 
drift, which $(1+x)^t$ instantiates, cannot accommodate large negative shocks 
driven by specific historical contingencies. The data refute this assumption. 
The independence property is therefore not just a mathematical abstraction: 
it is a testable claim about the generative process, and the evidence is 
against it.
...
OWID \citep{Ritchie2022} (1965, this work) & $4.95\,\mathrm{TW}$ & $\sim 1.2 \times 10^5\,H_0^{-1}$ (linear OLS) \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table*}
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=\textwidth]{figs/KSN_panel1_z.png}
    \caption{Zoomed view of the OWID global primary-energy production data 
    (1965--2024) with the linear and exponential best-fit models overlaid 
    on a linear vertical axis (W), making visible the remarkably low 
    dispersion of the data about both fitted models that is compressed 
    in the logarithmic-axis view of Figure~\ref{fig:ksn_panel1}. The 
    linear OLS fit (blue dashed, $R^2 = 0.987$) and the free-rate 
    exponential fit ($r = 2.01\%\,\mathrm{yr}^{-1}$; solid red, 
    $R^2 = 0.987$) are visually near-identical over the six-decade 
    baseline, confirming the WAIC result that the two functional forms 
    are statistically indistinguishable on the observed data. The 
    Kardashev one-percent exponential (salmon dotted) diverges 
    dramatically downward from the data, falsifying the conservative 
    assumption universally adopted in the subsequent literature. The 
    Kardashev Type~I threshold from 1964 (yellow dotted, 
    $4 \times 10^{12}\,\mathrm{W}$) lies well below the full dataset: 
    Earth has long since surpassed Kardashev's original Type~I energy 
    benchmark.}
    \label{fig:ksn_zoom}
\end{figure*}
...
\begin{figure}[t]
    ...
    \caption{Year-over-year differences 
$\Delta P_i = P_i - P_{i-1}$ in global primary-energy 
production (W) as a function of calendar year 1965--2024, 
derived from the OWID direct-method energy dataset. Two 
stray negative outliers --- clearly separated from the 
general trend of the data --- occur at 2008 and 2020, 
corresponding respectively to the subprime great financial 
crisis and the COVID-19 pandemic. These are identifiable 
historical shocks, not symmetric statistical fluctuations. 
The Shapiro--Wilk test on the $\Delta P$ series yields 
$W = 0.925$, $p = 0.0014$, with skewness $= -0.664$. 
The distribution is significantly non-Gaussian, and the 
identifiable crisis outliers constitute direct evidence 
of path-dependence --- incompatible with the structural 
assumptions of the Kardashev series $(1+x)^t$ (independent 
increments with positive drift).}
...
consistent with an independent-increment multiplicative generator
...
A further methodological consideration concerns per-year measurement 
uncertainties. The OWID dataset reports point estimates only; neither 
the Energy Institute nor the EIA publishes formal per-year uncertainty 
bands on global primary-energy production. Methodological considerations 
suggest that pre-1980 data carry larger fractional uncertainty 
($\sim 5$--$10\%$) than post-2000 data ($\sim 1$--$2\%$) due to 
incomplete reporting coverage in earlier decades, retroactive 
harmonisation of accounting methods, and imputation for missing 
country-year cells. The present analysis treats all 60 annual data 
points as having uniform relative uncertainty, which is the conservative 
default in the absence of published error bars. Plausible weighting 
schemes that down-weight pre-1980 data do not alter the qualitative 
conclusions of this work: the Kardashev $1\%$ exponential model is 
rejected by tens of standard deviations regardless of weighting; the 
linear-versus-exponential WAIC preference is robust to per-decade 
error assumptions; and the Shapiro--Wilk non-Gaussianity is driven 
primarily by the 2008 and 2020 outliers in well-measured recent 
decades, where any reasonable uncertainty estimate is small. A formal 
weighted MCMC with per-decade uncertainty assumptions is deferred to 
a future paper.
 
The Bitcoin network hashrate is the only publicly auditable, 
unforgeable, and continuously updated global measure of 
proof-of-work currently available. We do not consider Bitcoin 
as one network among many to be aggregated: Bitcoin commands the 
vast majority of total proof-of-work hashrate globally and has 
held this dominant position continuously since 2009. Network 
effects \citep{Metcalfe2013}, first-mover advantage, and the 
ossification of consensus rules have entrenched Bitcoin as the 
canonical proof-of-work substrate --- a pattern observed across 
foundational protocols, of which TCP/IP is the most prominent 
historical example: a "good enough" baseline standardised in 1981 
that has resisted displacement by technically superior alternatives 
for over four decades. Established systems benefit from accumulated 
infrastructure, security audits, developer expertise, and integration 
investments that newcomers cannot match, in the same way that 
established biological ecosystems resist invasion by 
marginally-more-optimised newcomers. We therefore treat Bitcoin 
as a representative measurement of global proof-of-work 
computation rather than as one constituent among many.
...
OWID energy repository \citep{Ritchie2022} identified and
...
\emph{references two distinct growth rates} in the same paper:
a projected empirical rate of $3$--$4\%\,\mathrm{yr}^{-1}$ 
attributed to \citet{Putnam1948} and projected over the following 
sixty years, and a conservative modelling assumption of 
$x = 1\%\,\mathrm{yr}^{-1}$ adopted for the Type~II timescale 
calculation. The standard Kardashev model universally cited 
in the literature \citep{gray2020} uses the conservative $1\%$ 
figure, not the Putnam-derived $3$--$4\%$ projection.
 
Our MCMC posterior growth rate of $r = 2.01\% \pm 0.03\%\,
\mathrm{yr}^{-1}$ falsifies both of Kardashev's reference values: 
the $1\%$ modelling assumption is too low, and the Putnam $3$--$4\%$ 
projection is too high. The true contemporary rate lies between 
the two values Kardashev referenced. This is not merely a 
falsification of \citet{kar64}; it is a falsification of the assumption, 
universally adopted in the subsequent literature, that a fixed 
exponential growth rate of any value could adequately describe 
the trajectory of the energy production of a typical civilisation.
...
detectable. Monte Carlo approaches to estimating the probability of causal contact between communicating civilisations \citep{Lares2020} provide a complementary framework in which the KSN variable may eventually constrain the longevity factor. This factor carries the largest relative
...
incompatible with the independent-increment
multiplicative structure with positive drift required by Kardashev's 
$(1+x)^t$ geometric series.
...
\textit{Kardashev's Conundrum}. Long before this date, the Sun 
will have exhausted its hydrogen fuel, expanded into a red 
giant, and likely engulfed or scorched the Earth. No functional form fitted
...
\citet{kar64} referenced two distinct growth rates: a
projected empirical rate of $3$--$4\%\,\mathrm{yr}^{-1}$
(attributed to \citealt{Putnam1948}) and a conservative
modelling assumption of $1\%\,\mathrm{yr}^{-1}$ adopted
for his Type~II extrapolation. Our posterior
$r = 2.01\%\,\mathrm{yr}^{-1}$ falsifies both simultaneously:
the $1\%$ assumption is too low, and the $3$--$4\%$
projection is too high. The true contemporary rate lies
between the two values Kardashev referenced --- a nuance
lost in the six decades of subsequent literature that
universally adopted the $1\%$ figure.

A speculative consequence of the Conundrum is that the original Kardashev Type II 
threshold is unreachable on any physically coherent timescale via energy 
scaling, so Type II energy-scaling technosignatures (in the Dyson-sphere 
sense) are unlikely ever to be observed; the KSN reframing preserves 
the Type II concept by relocating it to the Landauer-limit regime.
...
% =============================================================================
\section{Acknowledgments}
SG thanks R. Gamen from the VO of Argentina (NOVA) for the opportunity to serve as Argentina's representative on the IVOA Executive Committee (2015–2024). SG also thanks colleagues at the IATE and the OAC for their support. This work made use
