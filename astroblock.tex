...
\begin{abstract}
We test the standard Kardashev conjecture, which models the energy production
of technologically advanced civilisations as growing at a fixed rate of one
percent per year, against six decades of global energy production data
(1965--2024) drawn from the Our World in Data energy dataset. The standard
Kardashev one-percent exponential model is a poor fit to the data: the
posterior growth rate obtained via Markov Chain Monte Carlo (MCMC) inference
is $r = 1.85\% \pm 0.17\%\,\mathrm{yr}^{-1}$ (95\% credible interval
$[1.53\%, 2.19\%]$), placing the Kardashev one-percent value outside the
credible interval, and the fixed one-percent model residuals are substantially
larger than those of the linear alternative. A linear Ordinary Least-Squares
(OLS) model provides an exquisite fit ($R^2 = 0.983$) and is further
supported by the normality of year-on-year energy differences
(Shapiro--Wilk $W = 0.97$, $p = 0.23$), which is consistent with additive
Gaussian noise and formally rejects the multiplicative error structure
required by any exponential growth model. A free-parameter exponential model
and the linear model are statistically indistinguishable on the observed data
($\Delta\mathrm{WAIC} = 0.7$, $\Delta R^2 = 0.003$). However, extrapolation
of the linear model to the Kardashev Type~II threshold --- the total
bolometric luminosity of the Sun, $L_\odot = 3.828 \times
10^{26}\,\mathrm{W}$ --- yields a civilisational transition timescale of
$\sim 3.2 \times 10^{8}$ Hubble times: a physical reductio ad absurdum we
term \textit{Kardashev's Conundrum}. We demonstrate that no functional form
fitted to energy production alone can simultaneously satisfy statistical
adequacy and physical coherence, because the Kardashev state variable is
dimensionally incomplete as a measure of civilisational advancement. To
resolve the Conundrum, we propose the
\textit{Kardashev--Sagan--Nakamoto} (KSN) model, constructed by renormalising
the energy production ordinate by the annual average Bitcoin network hashrate
$H(t)$, yielding the KSN state variable $z = P/H$ in units of
J\,Hash$^{-1}$ (the KarNak unit). This renormalisation adds no free
parameters, is motivated by the Landauer limit connecting energy to
irreversible computation, and fulfils the original requirement that the
Kardashev scale be sensitive to the information richness of a civilisation.
The KSN variable spans 14 orders of magnitude over 2009--2024, reflecting the
dramatic improvement in the energy efficiency of Bitcoin proof-of-work
computation; a formal functional fit to $z(t)$ is deferred to a future work.
We further discuss the implications of the KSN model for open astronomy
development via decentralised blockchain protocols, and propose the KarNak
unit as a universal information-energy yardstick for the Kardashev scale.
...
timescale of $\sim 3.2 \times 10^{8}$ Hubble times: a physical
...
energy consumption in terra-watt-hours per year (TWh\,yr$^{-1}$), drawn from
...
figure ($592\,\mathrm{EJ}$) inconsistent with the historical OWID series.
Instead, we adopt the 2023 OWID value of $229{,}000\,\mathrm{TWh}$ and apply
the EI-reported annual growth rate of $2\%$ for 2024, giving
$233{,}580\,\mathrm{TWh}$, preserving methodological consistency throughout
the time series. All energy values are converted to SI watts (J\,s$^{-1}$)
via:
\begin{equation}
    P\,[\mathrm{W}] \;=\; E\,[\mathrm{TWh\,yr}^{-1}]
    \;\times\; \frac{10^{12}}{8760 \times 3600}\,,
    \label{eq:conversion}
\end{equation}
where the denominator converts one year to seconds. We set the origin of the
time coordinate at $t_0 = 1964$, the year of the Kardashev conjecture
\citep{Kardashev1964}, so that $t = \mathrm{year} - 1964$ throughout.
 
The Bitcoin network hashrate data $H(t)$ are yearly averages in hash\,s$^{-1}$,
compiled from the Blockchain.com public API and the NASDAQ BCHAIN/HRATE
dataset for 2009--2024. These span the full history of Bitcoin mining from the
CPU era ($\sim 3\,\mathrm{MH\,s^{-1}}$ in 2009) through the ASIC era
($\sim 620\,\mathrm{EH\,s^{-1}}$ in 2024), covering 16 annual data points
and 14 orders of magnitude in hashrate.
...
be reached in approximately $3{,}200$\,years. \citet{Sagan1973} obtained a
similar value using a slightly revised Type~II threshold of
$\sim 10^{26}\,\mathrm{W}$.
 
Figure~\ref{fig:ksn_notitle} shows the Kardashev model (Eq.~\ref{eq:kardashev})
alongside the OWID data on a logarithmic ordinate scale, together with the
Type~I and Type~II civilisation limits of \citet{Kardashev1964} and
\citet{Sagan1973}, and the solar insolation at Earth
($F_\oplus = L_\odot \pi R_\oplus^2 / 4\pi d_{\rm AU}^2
\approx 1.74 \times 10^{17}\,\mathrm{W}$). It is visually evident that the
one-percent Kardashev model diverges from the data: the data grow substantially
...
for the Kardashev model is $\sim 10^4$ times larger than for the linear model
defined in Section~\ref{sec:linear_model}, constituting overwhelming evidence
against the one-percent prior. We therefore conclude that the standard
Kardashev exponential model with $x = 0.01$ is statistically untenable.
 
%% -----------------------------------------------
\subsection{The Linear Model: an Exquisite Fit with a Physical Reductio}
...
$b = 8.68 \times 10^{7}\,\mathrm{W\,yr}^{-1}$ and coefficient of
determination $R^2 = 0.9827$. The fit is exquisite over the six-decade
...
normality test \citep{Shapiro1965}. We obtain $W = 0.97$, $p = 0.23$: we
cannot reject the null hypothesis that $\Delta P$ is drawn from a Normal
distribution at any standard significance level. The distribution of
$\Delta P$ has skewness $-0.31$ and is approximately symmetric, with outliers
at 2008 (subprime financial crisis) and 2020 (COVID-19 pandemic), both of
which represent identifiable exogenous shocks to global energy production.
 
This result has a decisive implication for model selection. The Shapiro--Wilk
result on $\Delta P$ is the statistical signature of \emph{additive Gaussian
noise}: the year-on-year increments are drawn from a distribution with
approximately constant variance, independent of the current level $P(t)$.
The exponential model (Eq.~\ref{eq:kardashev}), by contrast, requires that
increments are \emph{multiplicative} --- that $\Delta P / P$ is the
fundamental random variable --- producing variance that scales as $P^2$.
These are incompatible error structures.  The Shapiro--Wilk result therefore
formally rejects the generative mechanism of the exponential model at the
level of its assumed probability distribution, independent of goodness-of-fit
considerations.
 
Notwithstanding its exquisite in-sample fit, the linear model fails
catastrophically on extrapolation. Solving $P_{\rm L}(t^*) = L_\odot$ gives:
\begin{equation}
    t^* \;=\; \frac{L_\odot - a}{b}
    \;\approx\; 4.4 \times 10^{18}\,\mathrm{yr}
    \;\approx\; 3.2 \times 10^{8}\,H_0^{-1}\,,
    \label{eq:linear_typeii}
\end{equation}
where $H_0^{-1} \approx 13.8\,\mathrm{Gyr}$ is the Hubble time. The implied
Type~II timescale is $\sim 3.2 \times 10^{8}$ times the age of the Universe.
This is not merely improbable --- it is physically incoherent, since it
exceeds the main-sequence lifetime of the Sun by twelve orders of magnitude,
and the projected heat death of the Universe by many orders of magnitude.
We designate this result \textit{Kardashev's Conundrum}: the linear model,
which best fits the data by all standard statistical criteria, predicts
a civilisational transition timescale that is cosmologically nonsensical.
 
%% -----------------------------------------------
\subsection{Model Comparison and the Statistical Indistinguishability}
\label{sec:model_comparison}
 
To place the comparison on a rigorous Bayesian footing, we fit an exponential
model with the growth rate $r$ as a free parameter --- the ``Model~2 family''
of Section~\ref{sec:conundrum}:
\begin{equation}
    P_{\rm E}(t) \;=\; a_0\,e^{r t}\,,
    \label{eq:exp_free}
\end{equation}
...
posterior mean growth rate is $r = 1.85\% \pm 0.17\%\,\mathrm{yr}^{-1}$
(95\% credible interval $[1.53\%, 2.19\%]$), with the Kardashev value of
$1\%\,\mathrm{yr}^{-1}$ lying outside the $95\%$ credible interval.
The best-fit exponential model gives $R^2 = 0.9858$, marginally exceeding the
linear model ($R^2 = 0.9827$).
 
The Widely Applicable Information Criterion \citep[WAIC;][]{Watanabe2010}
comparison yields $\Delta\mathrm{WAIC} = \mathrm{WAIC}_{\rm linear} -
\mathrm{WAIC}_{\rm exp} = 0.7$: the two models are \emph{statistically
indistinguishable} on the observed data range. Since both models have
$K = 2$ free parameters, the AIC and BIC penalties are identical and also
provide no discrimination. This confirms and quantifies the statistical
statement of Kardashev's Conundrum: the data genuinely cannot discriminate
between the two functional forms over the observed baseline.
 
This indistinguishability is itself informative. Together with the Shapiro--Wilk
result rejecting the multiplicative error structure of the exponential model,
the complete statistical picture is:
\begin{enumerate}
    \item The one-percent Kardashev exponential is falsified by the data
          (poor fit, wrong error structure).
    \item The linear model and the free-$r$ exponential are
          statistically indistinguishable on observed data
          ($\Delta\mathrm{WAIC} = 0.7$).
    \item The linear model has the correct additive error structure
          (Shapiro--Wilk $p = 0.23$).
    \item The linear model is physically falsified by extrapolation
          (Type~II timescale $\sim 3.2 \times 10^8\,H_0^{-1}$).
\end{enumerate}
...
thermodynamic limit of computation. The variable $z = P / H$ thus inherits
physical meaning absent from $P$ alone.
 
\textbf{The renormalization.} We define the KSN state variable as:
\begin{equation}
    z(t) \;=\; \frac{P(t)}{H(t)}
    \qquad \left[\mathrm{J\,Hash}^{-1} \equiv \mathrm{KarNak}\right]\,,
    \label{eq:ksn}
\end{equation}
where $H(t)$ is the yearly average Bitcoin network hashrate in
...
This takes the analysis outside the Box--Cox family and is the
mathematical statement that neither the linear nor the exponential model
of $P(t)$ is the correct model: the correct extensive variable for the
Kardashev scale has not previously been identified. The KSN variable
$z(t)$ is our proposal.
 
Figure~\ref{fig:ksn_notitle} shows $z(t)$ for 2009--2024, spanning 16 annual
data points and 14 orders of magnitude in $z$. The data display a
monotonically decreasing trend consistent with an exponential decay:
\begin{equation}
    z(t) \;=\; z_0\,e^{-\gamma t}\,,
    \label{eq:ksn_model}
\end{equation}
with best-fit parameters $z_0 = 2.02 \times 10^3\,\mathrm{J\,Hash}^{-1}$
and decay rate $\gamma \approx 43\%\,\mathrm{yr}^{-1}$, giving a
characteristic timescale $\tau = 1/\gamma \approx 2.3\,\mathrm{yr}$
reflecting the rapid improvement in ASIC mining efficiency. A formal fit
to the KSN data is deferred to a future paper; Figure~\ref{fig:ksn_notitle}
presents the data and their trend as an open question for the community.
...
\toprule
Model & Growth rate & Type~II year & $\Delta t / H_0^{-1}$ \\
\midrule
Kardashev 1964 (fixed $r$) & $1.00\%\,\mathrm{yr}^{-1}$ & $\sim 5920$ & $\sim 3 \times 10^{-7}$ \\
Exp.\ free $r$ (MCMC mean) & $1.85\%\,\mathrm{yr}^{-1}$ & $\sim 4097$ & $\sim 2 \times 10^{-7}$ \\
Linear OLS & $8.68 \times 10^7\,\mathrm{W\,yr}^{-1}$ & $\sim 4.4 \times 10^{18}$ & $\sim 3.2 \times 10^{8}$ \\
KSN ($z = P/H$) & $\gamma \approx 43\%\,\mathrm{yr}^{-1}$ & \multicolumn{2}{l}{Re-expressed via Landauer limit} \\
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table*}
...
%% -----------------------------------------------
\subsection{The Memoryless Property as a Further Objection to the Exponential Model}
...
path-dependence. The slight negative skewness ($-0.31$) of the $\Delta P$
distribution is direct empirical evidence of this asymmetry --- large negative
shocks are systematically more probable than the symmetric memoryless
distribution would predict. The memoryless property is therefore not just a
mathematical abstraction: it is a testable claim about the process, and the
data reject it.
...
% (Old figure removed; replaced by the multi-panel KSN figure Fig.~\ref{fig:ksn_notitle})
%\begin{figure}
%    \centering
...
% (Removed Figures: OLS residual and relative residual plots; to be placed in second paper / appendix)
...
\begin{figure}
    \centering
    \IfFileExists{figs/KSN_notitle(1).png}{%
        \includegraphics[width=0.95\linewidth]{figs/KSN_notitle(1).png}%
    }{%
        \fbox{\parbox{0.95\linewidth}{\centering Missing figure file: \texttt{figs/KSN\_notitle(1).png}.}}%
    }
    \caption{Kardashev--Sagan--Nakamoto (KSN) diagnostic plot (three-panel figure).}
    \label{fig:ksn_notitle}
\end{figure}
...
First, the posterior growth rate $r = 1.85\% \pm 0.17\%\,
...
credible interval $[1.53\%, 2.19\%]$, the exponential component
of the KSN framework is falsified.

Second, the Shapiro--Wilk result ($W = 0.97$, $p = 0.23$)
establishes that year-on-year energy increments are consistent
with additive Gaussian noise. This is a testable claim: a
structural shift in the global energy system --- such as a
sustained period of multiplicative growth driven by a step
change in energy technology --- would produce a distribution
of $\Delta P$ that violates normality, falsifying the additive
error structure on which the linear model rests.
...
The KSN variable exhibits a structural break between the
CPU/GPU era (2009--2012) and the ASIC era (2013--2024),
reflecting a qualitative change in the technology of
proof-of-work computation rather than a smooth continuous
evolution. The log-space coefficient of determination
for a single exponential fit to the full $z(t)$ series
is $R^2_{\log} = 0.296$, which is poor, confirming that
a single functional form does not adequately describe the
full 2009--2024 baseline. A formal piecewise or
era-segmented fit is deferred to a future paper.

A further limitation concerns the production--consumption
distinction noted in Section~\ref{sec:intro}. The OWID
dataset reports total primary energy production, whereas
\citet{kar64} defined his scale in terms of energy
\emph{consumption}. We have argued that production
...
Our MCMC posterior growth rate of $r = 1.85\% \pm 0.17\%\,
\mathrm{yr}^{-1}$ sits precisely between these two values ---
above the conservative assumption but below the empirical
estimate. This is not a falsification of \citet{kar64}; it
is a vindication of his empirical judgment. The standard
``one-percent Kardashev model'' is falsified not because
Kardashev was wrong, but because the literature has been
testing a deliberately conservative lower bound rather than
...
risk: the dramatic decrease in $z(t)$ observed over
...
rate from MCMC inference is $r = 1.85\% \pm 0.17\%\,
\mathrm{yr}^{-1}$ (95\% credible interval $[1.53\%, 2.19\%]$),
placing the Kardashev one-percent value outside the credible
interval. The normality of year-on-year energy differences
(Shapiro--Wilk $W = 0.97$, $p = 0.23$) formally rejects
the multiplicative error structure required by the
exponential model.

A linear OLS model fits the data exquisitely ($R^2 = 0.983$)
and is statistically indistinguishable from a free-parameter
exponential model ($\Delta\mathrm{WAIC} = 0.7$,
$\Delta R^2 = 0.003$). However, extrapolation of the linear
model to the Kardashev Type~II threshold yields a
civilisational transition timescale of $\sim 3.2 \times 10^8$
...
$H(t)$, yielding the KSN state variable $z = P/H$ in
...
Our posterior $r = 1.85\%\,\mathrm{yr}^{-1}$ sits between
these values, vindicating Kardashev's empirical judgment
while falsifying the conservative assumption that the
literature has universally adopted.

The KSN variable spans 14 orders of magnitude over
2009--2024, reflecting the dramatic improvement in Bitcoin
proof-of-work energy efficiency from the CPU era through
to modern ASICs. A formal functional fit to $z(t)$,
including a piecewise treatment of the CPU/GPU and ASIC
eras, is deferred to a future work. The KarNak unit
is proposed as a universal information-energy yardstick
for the Kardashev scale, and the KSN framework as a
quantitative language for discussing civilisational
...
% =============================================================================
\section{Acknowledgments}
SG acknowledges the New Virtual Observatory of Argentina (NOVA) and his selection as the representative of Argentina on the IVOA-exec committee from 2015 to 2024 and support of all his colleagues in the IATE, especially those who administer the 'decentralized' supercomputer facility. This work made use of Astropy, a community-developed core Python package for Astronomy, and the broader open-source scientific software ecosystem (including SciPy and NumPy). The author sincerely thanks the Chinese Academy of Sciences for the MOU signed between the Astronomical Observatory of Cordoba and the Shanghai National Astronomical Observatory to value-add historical plate data of the old Argentina National Observatory.
