...
$P(1965) = 4.95\,\mathrm{TW}$, consistent with the \citet{Kardashev1964}
baseline of $4\,\mathrm{TW}$, and $P(2024) = 20.04\,\mathrm{TW}$,
...
normality test \citep{Shapiro1965}. We obtain $W = 0.918$, $p = 0.0007$:
...
skewness, driven by identifiable exogenous shocks to global energy production
...
\begin{equation}
    t^* \;=\; \frac{L_\odot - a}{b}
    \;\approx\; 1.57 \times 10^{15}\,\mathrm{yr}
    \;\approx\; 1.1 \times 10^{5}\,H_0^{-1}\,,
...
(95\% credible interval $[1.94\%, 2.07\%]$), with the Kardashev value of
$1\%\,\mathrm{yr}^{-1}$ lying well outside the $95\%$ credible interval.
The best-fit exponential model gives $R^2 = 0.987$, identical to the
...
\mathrm{WAIC}_{\rm linear} = 4.1$: the linear model is preferred over the
...
\begin{enumerate}
    ...
    \item The linear model and the free-$r$ exponential have
          ...
          preferred ($\Delta\mathrm{WAIC} = 4.1$).
    \item The year-on-year differences $\Delta P$ are non-Gaussian
          (Shapiro--Wilk $W = 0.918$, $p = 0.0007$), exhibiting
    ...
    \item The linear model is physically falsified by extrapolation
          (Type~II timescale $\sim 1.1 \times 10^5\,H_0^{-1}$).
\end{enumerate}
...
$z(t)$ is our proposal for the correct extensive variable of the
Kardashev scale.
...
these eras preclude a single-exponential description of $z(t)$; a formal
...
Exp.\ free $r$ (MCMC mean)  & $2.01\%\,\mathrm{yr}^{-1}$ & $\sim 3545$ & $\sim 1 \times 10^{-7}$ \\
Linear OLS                  & $2.44 \times 10^{11}\,\mathrm{W\,yr}^{-1}$ & $\sim 1.6 \times 10^{15}$ & $\sim 1.1 \times 10^{5}$ \\
...
The Shapiro--Wilk result ($W = 0.918$, $p = 0.0007$) provides direct
...
credible interval $[1.94\%, 2.07\%]$, the exponential component
of the KSN framework is falsified.
 
Second, the non-Gaussian structure of $\Delta P$
(Shapiro--Wilk $W = 0.918$, $p = 0.0007$) establishes that
...
evolution. A formal era-segmented fit to $z(t)$ is deferred
to a future paper.
...
\mathrm{yr}^{-1}$ (95\% credible interval $[1.94\%, 2.07\%]$),
...
(Shapiro--Wilk $W = 0.918$, $p = 0.0007$), with negative
skewness attributable to identifiable crisis events (2008, 2020),
...
($\Delta\mathrm{WAIC} = 4.1$). However, extrapolation of the
...
Our posterior $r = 2.01\%\,\mathrm{yr}^{-1}$ falls within
...
A formal era-segmented functional fit to $z(t)$, accounting
...
his colleagues in the IATE, especially those who administer
the decentralised supercomputer facility. This work made use
...
NumPy. The author sincerely thanks the Chinese Academy of
Sciences for the MOU signed between the Astronomical Observatory
of Cordoba and the Shanghai National Astronomical Observatory.
